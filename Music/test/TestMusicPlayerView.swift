    ...
    private var lyricDisplayView: some View {
        ScrollViewReader { proxy in
            ScrollView(showsIndicators: false) {
                VStack(spacing: 10) {
                    ForEach(Array(lyrics.enumerated()), id: \.offset) { index, obj in
                        Text(obj.content)
                            .font(.system(size: currentLyricIndex == index ? 20 : 12))
                            .foregroundColor(currentLyricIndex == index ? .primary : .gray)
                            .lineLimit(1)
                            .id(index)
                    }
                }
            }
            .onChange(of: currentLyricIndex) {
                withAnimation(.spring()) {
                    proxy.scrollTo(min(currentLyricIndex + 5, lyrics.count - 1), anchor: .bottom)
                }
            }
        }
    }
